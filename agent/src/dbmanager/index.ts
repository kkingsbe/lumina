...
export class DBWrapper {
    ...
    static async getMocByName(name: string): Promise<Option<Moc>> {
        ...
        if (mocData) {
            await DocumentModel.increment('access_count', { where: { id: mocData.id } });
    ...
    static async getDocumentMetadataById(id: string): Promise<Option<DocumentModel>> {
        const document = await DocumentModel.findOne({
            where: { id: id },
            attributes: ['id', 'name', 'file_location', 'access_count']
        });
        if (document) {
            await DocumentModel.increment('access_count', { where: { id: id } });
    ...
    static async getDocumentsByTitle(title: string): Promise<Document[]> {
        try {
            ...
            const documents = await Promise.all(documentModels.map(async (model) => {
                await DocumentModel.increment('access_count', { where: { id: model.id } });
    ...
    static async searchDocuments(options: FindOptions<DocumentModel>): Promise<DocumentModel[]> {
        try {
            ...
            // Increment access count for each found document
            await Promise.all(documents.map(doc => DocumentModel.increment('access_count', { where: { id: doc.id } })));
